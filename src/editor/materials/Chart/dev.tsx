    ...
    <div className="flex flex-col gap-3">
      {data.map((item) => (
        <div key={item.name} className="flex items-center gap-3">
          ...
          <div className="h-8 flex-1 rounded bg-slate-100 overflow-hidden">
            <div
    ...
    <div className="flex items-end gap-3 h-48">
      {data.map((item) => (
        <div key={item.name} className="flex flex-1 flex-col items-center gap-2">
          ...
          <div className="flex h-36 w-full items-end rounded bg-slate-50 px-2 pb-2">
            <div
...
export default function Chart({ id, title, chartType, dataText, dataSource, styles }: CommonComponentProps) {
  ...
  return (
    <div
      ...
      style={styles}
      className="rounded-xl border border-border-light bg-white p-4 shadow-soft"
    >

...
import { isMaterialDragItem, MATERIAL_DND_TYPE } from '../utils/material-dnd'

interface UseMaterialDropResult {
  canDrop: boolean
  dropRef: unknown
...
export function useMaterialDrop(id: number): UseMaterialDropResult {
  ...
  const [{ canDrop }, dropRef] = useDrop(() => {
    return {
      accept: MATERIAL_DND_TYPE,
      canDrop: (item) => isMaterialDragItem(item) && Boolean(componentConfig[item.componentName]),
      drop: (item, monitor) => { // 处理拖拽组件释放时的逻辑
        ...
        if (!isMaterialDragItem(item)) return
        messageApi.success(item.componentName)
        const component = createComponentInstance(componentConfig, item.componentName, Date.now())
  ...
  }, [componentConfig, addComponent, id])
